...
import MoneyTalk from './MoneyTalk';   // [zip54e] the Money Man's front door
...
export default function Nav({ screens, onLogout = () => {} }) {
  ...
  const openFromChat = (dest) => {
    ...
    if (dest.kind === 'moneytalk') return setOverlay({ tab: 'moneytalk' });   // [zip54e]
  ...
  if (overlay) {
    ...
    if (overlay.tab === 'moneytalk') return <MoneyTalk onBack={() => setOverlay(null)} onRun={(draft) => { setOverlay(null); navigate({ tab: 'gathering', persona: 'the_economist', draft, autoSend: true, from: 'moneytalk' }); }} onChat={() => { setOverlay(null); navigate({ tab: 'gathering', persona: 'the_economist', from: 'moneytalk' }); }} />;   // [zip54e]
