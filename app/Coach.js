...
import {
  ...
  coachMockStart, coachMockSubmit, coachLibrary, coachMaterial, API_BASE,
} from './api';
...
export default function Coach({ onBack = () => {}, onAskCoach = () => {} }) {
  ...
  const [entryStep, setEntryStep] = useState('pick');   // pick | library | custom
  const [subjects, setSubjects] = useState(null);        // house library list (null = not loaded)
  const [subjQ, setSubjQ] = useState('');
  const [pending, setPending] = useState(null);          // attached custom material { b64, uri }
  ...
  useBackLayer(
    (stage !== 'home' && stage !== 'loading') || (stage === 'home' && !course && entryStep !== 'pick'),
    useCallback(() => {
      if (stage === 'home' && !course && entryStep !== 'pick') { setEntryStep('pick'); setErr(''); return true; }
      setStage('home'); return true;
    }, [stage, course, entryStep]),
  );
  ...
  const startHouse = (subjectKey) => run('start', async () => {
    const c = await coachStart(null, 0, { mode: 'house', subject: subjectKey });
    await AsyncStorage.setItem(KEY, c.courseId);
    setCourse({ ...c, currentDay: 1, weakTags: [], days: {} });
    setEntryStep('pick'); setStage('home');
  });
  const startCustom = () => run('start', async () => {
    const t = topic.trim(); if (t.length < 2) { setErr('Name what you’re studying.'); return; }
    const c = await coachStart(t, Math.max(1, Math.min(parseInt(days, 10) || 7, 30)), { mode: 'custom' });
    if (pending?.b64) { try { await coachMaterial(c.courseId, 'material.jpg', pending.b64, 'image/jpeg'); } catch (e) {} }
    await AsyncStorage.setItem(KEY, c.courseId);
    setCourse({ ...c, currentDay: 1, weakTags: [], days: {} });
  ...
  const pickMaterial = async () => {
    try {
      ...
      if (!perm.granted) { setErr('Photo access is needed to attach your material.'); return; }
      const res = await ImagePicker.launchImageLibraryAsync({ mediaTypes: ['images'], quality: 0.5, base64: true });
      if (res.canceled || !res.assets?.[0]?.base64) return;
      setPending({ b64: res.assets[0].base64, uri: `data:image/jpeg;base64,${res.assets[0].base64}` });
    } catch (e) { setErr('Could not attach that image.'); }
  };
  ...
  const newCourse = async () => { await AsyncStorage.removeItem(KEY); setCourse(null); setTopic(''); setDays('7'); setAskA(null); setAsked(''); setEntryStep('pick'); setPending(null); setSubjects(null); setStage('home'); };
  ...
  const body = () => {
    ...
    if (stage === 'home') {
      if (!course) {
        ...
        if (entryStep === 'custom') return (
          <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }}>
            <ScrollView contentContainerStyle={s.scroll} keyboardShouldPersistTaps="handled">
              <Pressable onPress={() => { setEntryStep('pick'); setErr(''); }} hitSlop={10}><Text style={s.backLink}>‹ choose differently</Text></Pressable>
              <Text style={s.bigLead}>Bring your{'\n'}own material.</Text>
              <Text style={s.leadSub}>“Tell me what it is and hand me a page of it. I'll teach from your material, not the internet's.”</Text>
              <Text style={s.fieldLbl}>WHAT ARE YOU STUDYING</Text>
              <TextInput style={s.input} value={topic} onChangeText={setTopic} placeholder="e.g. my org-behaviour notes, this contract…" placeholderTextColor={P.faint} returnKeyType="done" />
              <Text style={s.fieldLbl}>YOUR MATERIAL</Text>
              <Pressable style={s.attach} onPress={pickMaterial}>
                {pending?.uri ? <Image source={{ uri: pending.uri }} style={s.thumb} /> : <Text style={s.attachIcon}>＋</Text>}
                <Text style={s.attachT}>{pending ? 'Attached — tap to replace' : 'Attach an image of your material'}</Text>
              </Pressable>
              <Text style={s.attachHint}>A clear photo of your notes or a page works. One page for now.</Text>
              ...
              <Pressable style={[s.cta, busy && s.ctaBusy]} onPress={startCustom} disabled={busy}>
                <Text style={s.ctaT}>{busyKey === 'start' ? 'Reading it…' : 'Build my plan'}</Text>
              </Pressable>
              <Err />
            </ScrollView>
        ...
        if (entryStep === 'library') {
          ...
          return (
            <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }}>
              <ScrollView contentContainerStyle={s.scroll} keyboardShouldPersistTaps="handled">
                <Pressable onPress={() => { setEntryStep('pick'); setErr(''); }} hitSlop={10}><Text style={s.backLink}>‹ choose differently</Text></Pressable>
                <Text style={s.bigLead}>Pick your{'\n'}subject.</Text>
                <Text style={s.leadSub}>“Each one's taught from our own shelves — a plan, day by day, grounded in the material.”</Text>
                ...
                {subjects === null
                  ...
                  : list.length === 0
                    ...
                    : list.map((x) => (
                        <Pressable key={x.key} style={s.subjRow} onPress={() => startHouse(x.key)} disabled={busy}>
                          <Text style={s.subjT}>{x.label}</Text>
                          <Text style={s.subjGo}>{busyKey === 'start' ? '…' : '→'}</Text>
                        </Pressable>
                      ))}
                <Err />
              </ScrollView>
...
const s = StyleSheet.create({
  ...
  backLink: { fontFamily: FONTS.semibold, color: P.muted, fontSize: 13, letterSpacing: 0.3, marginTop: 6, marginBottom: 2 },
  pick: { borderWidth: 1, borderColor: P.line, borderRadius: 16, backgroundColor: P.panel, paddingHorizontal: 18, paddingVertical: 18, marginTop: 16 },
  pickT: { fontFamily: FONTS.display, color: P.cream, fontSize: 21, letterSpacing: -0.3 },
  pickSub: { fontFamily: FONTS.body, color: P.muted, fontSize: 14, lineHeight: 20, marginTop: 7 },
  subjRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', borderBottomWidth: 1, borderBottomColor: P.line, paddingVertical: 16 },
  subjT: { fontFamily: FONTS.body, color: P.dim, fontSize: 16.5, flex: 1, paddingRight: 12 },
  subjGo: { fontFamily: FONTS.body, color: P.ember, fontSize: 18 },
  emptyList: { fontFamily: FONTS.displayItalic, color: P.faint, fontSize: 15, marginTop: 24 },
  attach: { flexDirection: 'row', alignItems: 'center', gap: 13, borderWidth: 1, borderColor: P.line, borderRadius: 15, backgroundColor: P.panel, paddingHorizontal: 16, paddingVertical: 14 },
  attachIcon: { fontFamily: FONTS.body, color: P.lamp, fontSize: 24, width: 30, textAlign: 'center' },
  attachT: { fontFamily: FONTS.body, color: P.dim, fontSize: 15, flex: 1 },
  attachHint: { fontFamily: FONTS.body, color: P.faint, fontSize: 12.5, marginTop: 9 },
  thumb: { width: 44, height: 44, borderRadius: 9, borderWidth: 1, borderColor: P.line },
